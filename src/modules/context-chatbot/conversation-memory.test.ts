...
describe("selectRelevantHistory", () => {
  ...
  it("recovers an older relevant exchange that a recency window would have dropped", async () => {
    ...
    const history = [
      user("wrong, planning officer does not belong to ECMS module"),
      assistant("understood, corrected"),
      ...Array.from({ length: 8 }, (_, i) => [user(`filler ${i}`), assistant(`reply ${i}`)]).flat(),
    ];
    const result = await selectRelevantHistory({
      history,
      question: "which roles belong to ECMS?",
      budgetTokens: 1_000,
      scoreExchanges: scoreByKeyword("ECMS"),
    });
    expect(result.map((m) => m.content)).toContain("wrong, planning officer does not belong to ECMS module");
  });

  it("keeps an answer with the question it answered", async () => {
    const history = [
      user("who approves a BCR?"), assistant("the OP manager approves it"),
      ...Array.from({ length: 6 }, (_, i) => [user(`filler ${i}`), assistant(`reply ${i}`)]).flat(),
    ];
    const result = await selectRelevantHistory({
      history,
      question: "BCR approval",
      budgetTokens: 1_000,
      scoreExchanges: scoreByKeyword("BCR"),
    });
    const contents = result.map((m) => m.content);
    // An assistant reply without its question reads as an answer to nothing.
    expect(contents).toContain("who approves a BCR?");
    expect(contents).toContain("the OP manager approves it");
  });

  it("returns messages in chronological order, never relevance order", async () => {
    const history = [
      user("first ECMS question"), assistant("first answer"),
      ...Array.from({ length: 6 }, (_, i) => [user(`filler ${i}`), assistant(`reply ${i}`)]).flat(),
    ];
    const result = await selectRelevantHistory({
      history,
    ...
    expect(contents.indexOf("first ECMS question")).toBeLessThan(contents.indexOf("filler 5"));
  });

  it("stops adding older exchanges once the budget is spent", async () => {
    const history = [
      ...Array.from({ length: 12 }, (_, i) => [user(`ECMS topic ${i} ${"x".repeat(400)}`), assistant("a")]).flat(),
    ];
    const generous = await selectRelevantHistory({
      history, question: "ECMS", budgetTokens: 5_000, scoreExchanges: scoreByKeyword("ECMS"),
    });
    const tight = await selectRelevantHistory({
      history, question: "ECMS", budgetTokens: 200, scoreExchanges: scoreByKeyword("ECMS"),
    });
